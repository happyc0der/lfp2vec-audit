...
@app.command("stage4-report")
def stage4_report(
    calibration: Path = typer.Option(Path("results/calibration")),
    abstention: Path = typer.Option(Path("results/abstention")),
    ablations: Path = typer.Option(Path("results/ablations")),
    out: Path = typer.Option(Path("docs/RESULTS_CALIBRATION.md")),
) -> None:
    """Assemble the calibration, abstention and ablation results into one document.

    Every number is read from the saved tables, so the document cannot drift from the experiments
    that produced it and regenerates from a clean checkout.
    """
    direction = {
        "cross_lab_ibl_to_allen__all_target_groups__seed0": "IBL to Allen",
        "cross_lab_allen_to_ibl__all_target_groups__seed0": "Allen to IBL",
    }
    short = {
        "cross_lab_ibl_to_allen": "IBL to Allen",
        "cross_lab_allen_to_ibl": "Allen to IBL",
    }
    ...
    reports = {f.stem: json.loads(f.read_text()) for f in sorted(calibration.glob("*.json"))}
    if reports:
        lines += [
        ...
        for run, payload in sorted(reports.items()):
            name = direction.get(run, run)
            for split, label in (("in_lab", "in lab"), ("cross_lab", "cross lab")):
                d = payload[split]
                lines.append(
                    f"| {name} | {label} | 1.00 → {d['temperature']:.2f} | "
                    f"{d['ece_before']:.3f} → {d['ece_after']:.3f} | "
                    f"{d['nll_before']:.3f} → {d['nll_after']:.3f} | "
                    f"{d['mean_confidence_before']:.3f} → {d['mean_confidence_after']:.3f} | "
                    f"{d['accuracy']:.3f} |"
                )
            o = payload["cross_lab_oracle"]
            lines.append(
                f"| {name} | cross lab, oracle | 1.00 → {o['temperature']:.2f} | "
                f"{o['ece_before']:.3f} → {o['ece_after']:.3f} | "
                f"{o['nll_before']:.3f} → {o['nll_after']:.3f} | "
                f"{o['mean_confidence_before']:.3f} → {o['mean_confidence_after']:.3f} | "
                f"{o['accuracy']:.3f} |"
            )
        gaps = [
            f"{direction.get(r, r)} needed {p['temperature_oracle']:.2f} against an in-lab fit of "
            f"{p['temperature_in_lab']:.2f}, a factor of "
            f"{p['temperature_oracle'] / p['temperature_in_lab']:.1f}"
            for r, p in sorted(reports.items())
        ]
        lines += [
            "",
            "In both directions the in-lab fit works and the same scalar barely moves the",
            "cross-lab number. " + "; ".join(gaps) + ".",
            "",
    ...
    frames = [pd.read_csv(f) for f in sorted(abstention.glob("*.csv"))]
    if frames:
        table = pd.concat(frames, ignore_index=True)
        lines += [
        ...
        for run, part in table.groupby("run"):
            for row in part.itertuples():
                lines.append(
                    f"| {direction.get(run, run)} | {row.score} | {row.aurc:.3f} | "
                    f"{row.risk_full:.3f} | {row.risk_fifth:.3f} | {row.in_vs_out_auc:.3f} |"
                )
        lines += [
            "",
    ...
    frames = [pd.read_csv(f) for f in sorted(ablations.glob("*.csv"))]
    if frames:
        table = pd.concat(frames, ignore_index=True)
        lines += [
        ...
        for scheme, part in table.groupby("scheme"):
            piv = part.pivot_table(index="ablation", columns="model", values="delta")
            ref = part[part.ablation == "none"].set_index("model")["balanced_accuracy"]
            models = [m for m in ("bandpower", "frozen", "finetuned") if m in piv.columns]
            order = [a for a in part.ablation.unique() if a != "none"]
            lines += [
                f"### {short.get(scheme, scheme)}",
                "",
                "| transform | " + " | ".join(m for m in models) + " |",
                "|---" * (len(models) + 1) + "|",
                "| unmodified | " + " | ".join(f"{ref[m]:.3f}" for m in models) + " |",
            ]
            for name in order:
                cells = " | ".join(f"{piv.loc[name, m]:+.3f}" for m in models)
                lines.append(f"| {name.replace('_', ' ')} | {cells} |")
            lines.append("")
...
@app.command("position-report")
def position_report(
    results: Path = typer.Option(Path("results/position")),
    baselines: Path = typer.Option(Path("results/baselines")),
    out: Path = typer.Option(Path("docs/RESULTS_POSITION.md")),
) -> None:
    """Assemble the position study into one document, from saved tables only."""
    order = ["loso_ibl", "loso_allen", "cross_lab_ibl_to_allen", "cross_lab_allen_to_ibl"]
    names = {
        "loso_ibl": "within IBL",
        "loso_allen": "within Allen",
    ...
    folds_path = Path(baselines) / "folds.csv"
    if folds_path.exists():
        ...
        pivot = part.pivot_table(index="features", columns="scheme", values="balanced_accuracy")
        chance = part.groupby("scheme")["chance"].mean()
        lines += [
            ...
            "",
            "| features | " + " | ".join(names[k] for k in order) + " |",
            "|---|" + "---:|" * len(order),
            "| *chance* | " + " | ".join(f"*{chance[k]:.3f}*" for k in order) + " |",
        ]
        for key, label in (
            ("position", "**electrode position (honest)**"),
            ("position_leaky_span", "relative depth within labelled span *(leaks test labels)*"),
        ):
            if key in pivot.index:
                cells = " | ".join(f"{pivot.loc[key, k]:.3f}" for k in order)
    ...
    fusion_path = Path(results) / "fusion_folds.csv"
    if fusion_path.exists():
        fusion = pd.read_csv(fusion_path)
        pivot = fusion.pivot_table(index="source", columns="scheme", values="balanced_accuracy")
        rows = [
            ("position", "position"),
            ("bandpower_full", "band power"),
            ("w2v2_frozen", "frozen audio model"),
            ("position+bandpower_full", "position × band power"),
            ("position+w2v2_frozen", "position × frozen audio model"),
        ]
        lines += [
            ...
            "",
            "| source | " + " | ".join(names[k] for k in order) + " |",
            "|---|" + "---:|" * len(order),
        ]
        for key, label in rows:
            if key in pivot.index:
                lines.append(
                    f"| {label} | " + " | ".join(f"{pivot.loc[key, k]:.3f}" for k in order) + " |"
                )
        lines.append("")
        for scheme in ("loso_ibl", "loso_allen"):
            wide = fusion[fusion["scheme"] == scheme].pivot_table(
                index="fold", columns="source", values="balanced_accuracy"
            )
            over_position = wide["position+w2v2_frozen"] - wide["position"]
            over_signal = wide["position+w2v2_frozen"] - wide["w2v2_frozen"]
            lines.append(
                f"- **{names[scheme]}**: fusing adds {over_position.mean():+.3f} to position "
                f"({int((over_position > 0).sum())} of {len(over_position)} sessions) and "
                f"{over_signal.mean():+.3f} to the signal "
                f"({int((over_signal > 0).sum())} of {len(over_signal)})."
            )
        lines += [
            "",
    ...
    depth_path = Path(results) / "depth_uncertainty.csv"
    if depth_path.exists():
        ...
        for scheme in ("loso_ibl", "loso_allen"):
            mean = (
                ...
                .groupby("half_width_um")[["position", signal, f"position+{signal}"]]
                .mean()
            )
            lines += [
                f"**{names[scheme]}**",
            ...
            for width, row in mean.iterrows():
                label = "exact" if width == 0 else f"±{int(width)} µm"
                lines.append(
                    f"| {label} | {row['position']:.3f} | {row[signal]:.3f} | "
                    f"{row[f'position+{signal}']:.3f} |"
                )
            crossing = None
            widths = list(mean.index)
            for a, b in zip(widths[:-1], widths[1:], strict=True):
                ...
                if ya >= target > yb:
                    crossing = a + (ya - target) / (ya - yb) * (b - a)
                    break
            if mean.loc[widths[0], "position"] < mean.loc[widths[0], signal]:
                lines += [
                    "",
                ...
                lines += [
                    "",
                    f"The signal becomes the better single source beyond about ±{crossing:.0f} µm,",
    ...
    boundary_path = Path(results) / "boundary.csv"
    if boundary_path.exists():
        ...
        for scheme in ("loso_ibl", "loso_allen"):
            part = boundary[boundary["scheme"] == scheme]
            lines += [
                f"**{names[scheme]}**",
                "",
            ...
            for _, row in part.iterrows():
                lines.append(
                    f"| {row['bin']} | {int(row['n'])} | {row['position']:.3f} | "
                    f"{row['w2v2_frozen']:.3f} | {row['position+w2v2_frozen']:.3f} |"
                )
            lines.append("")
        lines += [
